...
var compareToEquals = collections.compareToEquals;
var defaultCompare = collections.defaultCompare;
var defaultEquals = collections.defaultEquals;
/** Graph module
*
...
function biDirectionalSearch(graph, start, goal, heuristics, timeout) {
    var result = {
        path: [start],
        cost: 0
    };
    ...
    var costNodesEqual = function (first, second) {
    ...
    function getGoalNodes(usedNodes, actual, goalNodes) {
        for (var _i = 0, _a = graph.outgoingEdges(actual); _i < _a.length; _i++) {
            var edge = _a[_i];
    ...
    var openNodesStart = new Heap(compareFValue, costNodesEqual);
    var openNodesGoal = new Heap(compareFValue, costNodesEqual);
    // Add start to the set of open nodes, including setting its g-cost and predecessor.
    var startNode = new CostNode(start, 0, 0);
    ...
    while (!openNodesStart.isEmpty() && !openNodesGoal.isEmpty()) {
        ...
        for (var _i = 0, _a = graph.outgoingEdges(currentStartNode.node); _i < _a.length; _i++) {
            var edge = _a[_i];
            if (!closedStartNodes.contains(edge.to)) {
                var node = openNodesStart.getElement(new CostNode(edge.to, 0, 0));
                if (node == null) {
                    node = new CostNode(edge.to, currentStartNode.getGCost() + edge.cost, heuristics(edge.to));
...
var CostNode = (function () {
    function CostNode(node, gCost, hCost) {
        this.node = node;
        this.hCost = hCost;
        this.setGCost(gCost);
    }
    CostNode.prototype.setGCost = function (gCost) {
        this.gCost = gCost;
        this.fCost = gCost + this.hCost;
    };
    CostNode.prototype.getGCost = function () {
        return this.gCost;
    };
    CostNode.prototype.toString = function () {
        return "" + this.node;
    };
    return CostNode;
}());
...
function aStarSearch(graph, start, goal, heuristics, timeout) {
    ...
    // The set of open nodes.
    var openNodes = new Heap(compareFValue, costNodesEqual);
    // Add start to the set of open nodes, including setting its g-cost and predecessor.
    var startNode = new CostNode(start, 0, 0);
    predecessors.setValue(startNode, startNode);
    ...
    while (!openNodes.isEmpty()) {
        ...
        // Goes through every neighbouring node.
        for (var _i = 0, _a = graph.outgoingEdges(currentNode.node); _i < _a.length; _i++) {
            var edge = _a[_i];
            ...
            if (!closedNodes.contains(edge.to)) {
                ...
                var node = openNodes.getElement(new CostNode(edge.to, 0, 0));
                if (node == null) {
                    node = new CostNode(edge.to, currentNode.getGCost() + edge.cost, heuristics(edge.to));
...
var Heap = (function () {
     ...
     *                                               two nodes are equal
     */
    function Heap(compareFunction, equalsFunction) {
        this.data = [];
     ...
     */
    Heap.prototype.update = function (element) {
     ...
     *                           otherwise
     */
    Heap.prototype.getElement = function (element) {
